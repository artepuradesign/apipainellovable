  ...
  try {
    ...
    if (searchResult.success && searchResult.data) {
      ...
      try {
        ...
        const moduleTypeTitle = (metadata?.module_title || metadata?.moduleTypeTitle || 'NOME COMPLETO').toString();
        ...
        const registroPayload = {
          ...
          saldo_usado: saldoUsado, // Incluir o tipo de saldo usado
            metadata: {
              ...
              module_id: 156, // ID do módulo Nome Completo
      ...
      if (preCheck.success && preCheck.exists && preCheck.data) {
        ...
        const registroPayload = {
          ...
          saldo_usado: saldoUsado,
          metadata: {
            source: 'consultar-nome-completo-precheck',
            page_route: window.location.pathname,
            module_title: moduleTypeTitle,
            discount: metadata.discount || 0,
        ...
        if (atitoResult.success) {
          ...
          if (finalCheck.success && finalCheck.exists && finalCheck.data) {
            ...
            const registroPayload = {
              ...
              saldo_usado: saldoUsado,
              metadata: {
            source: 'consultar-nome-completo-railway-flow',
                page_route: window.location.pathname,
                module_title: moduleTypeTitle,
                discount: metadata.discount || 0,
                original_price: metadata.original_price || finalCost,
...
const ConsultarNomeCompleto = () => {
    ...
    try {
      ...
      if (response.success && response.data && Array.isArray(response.data)) {
        const getModuleLabel = (route?: string) => {
          ...
          if (!r) return '-';
          if (r.includes('/dashboard/consultar-nome-completo')) return 'NOME COMPLETO';
  ...
  const handleSearch = async () => {
    console.log('🚀 [HANDLE_SEARCH] INÍCIO da consulta Nome Completo');
    
    // Validações prévias detalhadas - Nome deve ter pelo menos 3 caracteres
    if (!cpf || cpf.trim().length < 3) {
      console.error('❌ [HANDLE_SEARCH] Nome inválido:', cpf);
    ...
    <div className="space-y-4 md:space-y-6 max-w-full overflow-x-hidden">
      <div className="w-full">
        <SimpleTitleBar
          ...
          subtitle="Consulte dados pelo Nome Completo na base de dados"
        ...
        <Card className="dark:bg-gray-800 dark:border-gray-700 w-full">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="nomeCompleto">Nome Completo (para busca)</Label>
              <Input
                id="nomeCompleto"
                ...
                onKeyPress={(e) => {
                  if (e.key === 'Enter' && cpf.trim().length >= 3 && !loading && hasSufficientBalance(finalPrice) && !modulePriceLoading) {
                    handleSearch();
                  }
                }}
                autoFocus
            ...
            <div className="flex flex-col gap-3">
              <Button
                ...
                disabled={loading || !cpf || cpf.trim().length < 3 || !hasSufficientBalance(finalPrice) || modulePriceLoading}
            ...
            {!hasSufficientBalance(finalPrice) && cpf.trim().length >= 3 && (
